...
/**
 * Relays PENDING outbox rows to Kafka on a fixed delay. Runs in its own transaction per batch:
 * a row is only marked PUBLISHED after {@link KafkaTemplate#send} completes, so a Kafka outage
 * simply leaves rows PENDING for the next poll instead of losing them or double-committing
 * partial work. {@code @Retry} covers transient broker unavailability within one poll; a row
 * that keeps failing accumulates {@code retryCount} and is surfaced via
 * {@code /actuator/metrics} rather than silently dropped.
 */
@Component
